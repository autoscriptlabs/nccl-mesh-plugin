#!/usr/bin/env python3
"""
Optimized Qwen2.5-32B training for 3-node DGX Spark cluster (unified memory)

Memory budget per node: 117GB
Total cluster memory: 351GB

Key optimizations:
- 8-bit Adam: Reduces optimizer states from 256GB → 128GB
- Gradient checkpointing: Trades compute for activation memory
- Small micro-batch + gradient accumulation: Controls peak memory
- BF16 training: Half precision throughout

Usage:
    srun -N3 --ntasks-per-node=1 torchrun --nproc_per_node=1 \
        --nnodes=3 --rdzv_backend=c10d --rdzv_endpoint=$HEAD:29500 \
        train_qwen32b_optimized.py
...
def get_model_config():
    ...
    return {
        ...
        # Training
        "micro_batch_size": 1,          # Per-device batch size (keep small!)
...
def get_optimizer(model, config):
    ...
    if config["use_8bit_adam"] and HAS_BNB:
        ...
        # Reduces optimizer memory from 256GB → 128GB for 32B model
...
def main():
    ...
    if accelerator.is_main_process:
        print("=" * 60)
        print("OPTIMIZED QWEN-32B TRAINING")
        print("=" * 60)
